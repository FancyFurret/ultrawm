...
# Copy icon for macOS (ICNS should be generated by build.rs during cargo build)
ICON_NAME=""
ICNS_PATH=$(find target/release/build -name "icon.icns" -type f 2>/dev/null | head -n 1)

if [ -n "${ICNS_PATH}" ] && [ -f "${ICNS_PATH}" ]; then
    echo "Copying icon.icns from ${ICNS_PATH}..."
